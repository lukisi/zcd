...
void metronome()
{
    ...
    string msg_request = @"{\"unicast-request\":{\"ID\":$(request_id),\"request\":{\"unicast-id\":{\"typename\":\"NetsukukuUnicastID\",\"value\":{\"id\":$(test_unicast_id)}},\"source-id\":{\"typename\":\"NetsukukuSourceID\",\"value\":{\"id\":$(test_source_id)}},\"wait-reply\":true,\"method-name\":\"my_app.do_stuff\",\"arguments\":[{\"argument\":1},{\"argument\":\"ab\"}]}}}";
    ch_server_gets_request.send(msg_request);
    ch_metronome.recv();
    status = 1;
    print("metronome: the message has been received. Just wait for completion.\n");
    ch_server_sends_reply.recv();
}
...
void main()
{
     ...
     * A program prepares to listen to requests on UDP port 269. It receives a request and correctly reacts calling the IZcdUdpRequestMessageDelegate that has been passed. Then, it correctly sends back the response.
     */
...
class MyDispatcher : Object, IZcdDispatcher
{
    public string execute()
    {
        ...
        print("Dispatcher.execute: we fake a job long enough to get a KEEPALIVE.\n");
...
class MyUdpRequestMessageDelegate : Object, IZcdUdpRequestMessageDelegate
{
    ...
    {
        print(@"get_dispatcher_unicast($(id)):\n");
        print(@" unicast_id: '$(unicast_id)'\n");
        print(@" m_name: '$(m_name)'\n");
